...
pub fn ReplicaType(
    ...
    return struct {
        ...
        /// Highest commit_min from backups, tracked via prepare_ok messages.
        ...
        fn commit_stall(self: *Replica) enum { ready, pending } {
            ...
            const commit_min_min = commit_min_min: {
                ...
                for (self.commit_mins[0..self.replica_count]) |commit_min| {
                    ...
                    // two checkpoints behind, they may be down or partitioned.
                    // We choose two checkpoints as in that case a lagging
                    // replica might still be able to catch up via WAL repair.
                    // Otherwise, it must use state sync (see `on_view_set_checkpoint`).
                    if (self.commit_min -| commit_min <= 2 * constants.vsr_checkpoint_ops) {
            ...
            assert(commit_lag <= 2 * constants.vsr_checkpoint_ops);
